...
const getBatteryIcon = (chargePercent: number): string => {
  ...
  return threshold?.icon || BATTERY_THRESHOLDS[BATTERY_THRESHOLDS.length - 1].icon;
};

const getVolumeIcon = (volume: number): string => {
  const threshold = VOLUME_THRESHOLDS.find((t) => volume >= t.min);
  return threshold?.icon || VOLUME_THRESHOLDS[VOLUME_THRESHOLDS.length - 1].icon;
};
...
const iconMapTyped = (iconMap as IconMapEntry[]).map((item) => ({
  ...item,
  normalizedNames: item.appNames.map(normalizeAppName),
}));

const getSpecialAppIcon = (title: string, exe: string): string | null => {
  const normalizedTitle = title.toLowerCase();

  switch (exe.toLowerCase()) {
    case "olk":
      // Main Outlook window vs individual message windows
      return normalizedTitle.endsWith(" - outlook") ? ":microsoft_outlook:" : ":mail:";
...
const getAppIcon = (title: string, exe: string): string => {
  const specialIcon = getSpecialAppIcon(title, exe);
  if (specialIcon) {
    return specialIcon;
  }
...
const calculateDiskUsage = (disk: Disk): number => {
  ...
  return Math.round(100 - (disk.availableSpace.iecValue / disk.totalSpace.iecValue) * 100);
};

const WorkspaceButton = (props: { workspace: Workspace; glazewm: GlazeWmOutput }) => {
  const workspaceWindows = createMemo(() => {
    if (!props.glazewm?.allWindows || !props.workspace) return [];
    return props.glazewm.allWindows.filter((window) => window.parentId === props.workspace.id);
  });
  ...
  return (
    <button
      ...
      onClick={() => props.glazewm.runCommand(`focus --workspace ${props.workspace.name}`)}
    >
      {props.workspace.name}
      <Show when={workspaceWindows().length > 0} fallback={<span>—</span>}>
        <For each={workspaceWindows()}>
          {(window) => (
            <span class="sketchy-icon">
...
const TeamsStatus = () => {
  ...
  return (
    <div class={isConnected() ? "teams-status connected" : "teams-status disconnected"}>
...
const SystemStats = (props: {
  ...
  return (
    <div class="stats">
      ...
      <Show when={props.cpu}>
        <div class="cpu">
          <span class={props.cpu.usage > HIGH_CPU_THRESHOLD ? "high-usage" : ""}>
        ...
        <Show when={output.glazewm}>
          ...
          <div class="workspaces">
            <For each={output.glazewm.currentWorkspaces}>
              {(workspace) => <WorkspaceButton workspace={workspace} glazewm={output.glazewm} />}
            </For>
          </div>
      ...
      <div class="right">
        <div class="media-container">
          ...
          <Show when={output.audio?.defaultPlaybackDevice}>
            <i class={getVolumeIcon(output.audio.defaultPlaybackDevice.volume)} />
          </Show>
        </div>
