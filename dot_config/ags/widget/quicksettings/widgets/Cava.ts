	...
	Widget.DrawingArea({
		...
		attribute: {
			cavaVar: Variable([], {
				listen: [
					...
					(out: string) => out.split(";").slice(0, -1).map(Number),
				],
			}),
		},
		setup(self) {
			if (vertical) self.set_size_request(height, bars);
			else self.set_size_request(bars, height);
		...
		if (!smooth) {
			for (let i = 0; i < self.attribute.cavaVar.value.length; i++) {
				...
				if (vertical) cr.rectangle(x, i * (h / bars), barHeight, h / bars);
				else cr.rectangle(i * (w / bars), y, w / bars, barHeight);
				cr.fill();
			}
		} else {
			let lastX = 0;
