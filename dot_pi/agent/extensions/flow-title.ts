...
const PALETTE: Rgb[] = [MAUVE, LAVENDER, BLUE, SAPPHIRE, SKY, SAPPHIRE, BLUE, LAVENDER];
...
export default function (pi: ExtensionAPI) {
  ...
  function installHeader(ctx: ExtensionContext) {
    ...
    ctx.ui.setHeader((tui) => {
      ...
      return {
        render(width: number) {
          return renderHeader(width, phase, `${currentModelId} · ${projectName()}`);
        },
        invalidate() {
          tui.requestRender();
        },
  ...
  pi.registerCommand("flow-title", {
    description: "Enable the Catppuccin flowing gradient session header",
